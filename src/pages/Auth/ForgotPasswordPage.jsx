...
const ForgotPasswordPage = () => {
  ...
  const handleVerifyOtp = async (e) => {
    ...
    setLoading(true);
    try {
      const result = await authService.vaultResetVerify(email.trim(), otp.trim());
  ...
  const handleResetComplete = async (e) => {
    ...
    try {
      ...
      const masterKey = await unlockMasterKeyWithRecovery(
        recoveryPhrase.trim(), params.recoveryEncryptedMasterKey, params.recoverySalt, params.recoveryKdfParams
      );
      ...
      await authService.vaultResetComplete({ resetToken, newPassword, newEncryptedMasterKey, newKekSalt });
      setStep('success');
    } catch (err) {
  ...
  const inputClass = 'w-full px-4 py-3 bg-[#0a0a0a] border border-gray-800 rounded-lg text-white placeholder-gray-500 focus:outline-none focus:border-blue-500 transition-colors';
  const btnClass = 'w-full py-3 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors disabled:opacity-50 disabled:cursor-not-allowed';
          ...
          <>
            ...
            <form onSubmit={handleVerifyOtp} className="space-y-4">
              <div>
                ...
                <input type="text" placeholder="Enter 6-digit code" value={otp} maxLength={6}
                  onChange={(e) => setOtp(e.target.value.replace(/\D/g, ''))} className={`${inputClass} text-center text-2xl  font-mono`}
                  required autoFocus />
              </div>
        ...
        {step === 'recovery' && (
          <>
            ...
            <form onSubmit={handleResetComplete} className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1.5">Recovery Key</label>
                <input type="text" placeholder="xxxx-xxxx-xxxx-xxxx-..." value={recoveryPhrase}
                  onChange={(e) => setRecoveryPhrase(e.target.value)} className={`${inputClass} font-mono text-sm`}
                  required autoFocus />
                <p className="text-xs text-gray-500 mt-1">The key you saved when you created your account</p>
              </div>
              <div>
                ...
                <input type="password" placeholder="At least 8 characters" value={newPassword}
                  onChange={(e) => setNewPassword(e.target.value)} className={inputClass} required minLength={8} />
              </div>
              <div>
