...
// Annual pricing configuration (matching PlanSelector)
const ANNUAL_PRICES = {
  starter: 70,
  basic: 130,
  plus: 250,
...
const SubscriptionSection = () => {
  ...
  const [paymentType, setPaymentType] = useState('instapay');
  ...
  const handleSelectPlan = async (planId, options = {}) => {
    setError('');
    ...
    // Create an enhanced plan object with pricing information
    const enhancedPlan = {
      ...plan,
      isAnnual: options.isAnnual || false,
      priceEgp: options.isAnnual ? ANNUAL_PRICES[plan.name] || plan.priceEgp : plan.priceEgp,
      billingPeriod: options.isAnnual ? 'yearly' : 'monthly',
    };
    setPendingPlan(enhancedPlan);
    ...
    <div className="space-y-6">
              ...
              <div className="flex-1">
                ...
                {instapaySubmission.status === 'pending_verification' && (
                  <p className="opacity-80 mt-1">
                    We'll verify and send you an email with confirmation.
                  </p>
                )}
                {instapaySubmission.status === 'rejected' &&
      ...
      <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-sm border border-gray-200 dark:border-zinc-700 p-6">
        ...
        {subscription ? (
          ...
          <div className="text-center py-8">
            ...
            <button
              onClick={() => setShowPlanSelector(true)}
              className="text-sm text-green-500 dark:text-green-700 mb-4 bg-green-200 p-2 rounded-md"
            >
              Upgrade Now
            </button>
          </div>
      ...
      {showMethodChooser && pendingPlan && (
        <PaymentMethodChooser
          ...
          onPickBankTransfer={handlePickBankTransfer}
