...
function PlanCard({ plan, isCurrent, isAnnual }) {
    ...
    <div className="w-full shrink-0 px-2">
      ...
      >
        ...
        <p className="mt-1.5 mb-5 text-sm text-gray-600 dark:text-gray-300">
          EGP {isAnnual ? ANNUAL_PRICES[plan.name] || plan.priceEgp : Number(plan.priceEgp).toFixed(0)}
          <span className="text-gray-500 dark:text-gray-400">/{isAnnual ? 'yr' : 'mo'}</span>
...
function ConfirmScreen({ plan, onBack, onConfirm, loading, isAnnual }) {
  const accent = getAccent(plan.name);
  return (
    <div className="text-center px-4 py-8">
      ...
      <p className="m-0 mb-2 text-sm text-gray-600 dark:text-gray-300">
        EGP {isAnnual ? ANNUAL_PRICES[plan.name] || plan.priceEgp : Number(plan.priceEgp).toFixed(0)}/{isAnnual ? 'yr' : 'mo'} &middot;{' '}
        {fmtStorage(plan.storageGb)}
...
export default function PlanSelector({
  ...
  async function handleConfirm() {
    setLoading(true);
    try {
      await onSelect(plan.id, { isAnnual });
    } finally {
      setLoading(false);
    }
  }
      ...
      <div className="relative z-10 w-full max-w-md bg-white dark:bg-zinc-900 rounded-2xl border border-gray-200 dark:border-zinc-700 overflow-hidden shadow-2xl">
        ...
        <div className="flex items-center justify-center gap-3 px-6 pt-4">
          ...
          <button
            ...
            className="relative inline-flex h-6 w-11 items-center rounded-sm transition-colors focus:outline-none focus:ring-2 focus:ring-primary-500 focus:ring-offset-2"
            style={{ backgroundColor: isAnnual ? '#10b981' : '#d1d5db' }}
          >
            <span
              className="inline-block h-4 w-4 transform rounded-full bg-white transition-transform"
              style={{ transform: isAnnual ? 'translateX(1.25rem)' : 'translateX(0.25rem)' }}
            />
          </button>
          <span className={`text-sm font-medium ${
            ...
            <div className="px-6 pt-5 pb-6 flex flex-col gap-2">
              ...
              >
                {isCurrent
                  ...
                  : `Select ${plan?.displayName ?? ''} — EGP ${isAnnual ? ANNUAL_PRICES[plan?.name] || plan?.priceEgp : Number(plan?.priceEgp ?? 0).toFixed(0)}/${isAnnual ? 'yr' : 'mo'}`}
              </button>
